...
class USER(QDialog):        # Dialog box for entering name and key of new dataset.
    ...
    def get_name_key(self):
        ...
        bday = self.dateEdit.text()
        return name, key, email, bday
...
class AUFR(QMainWindow):        # Main application 
    ...
    def __init__(self):
        ...
        # Variables
        self.camera_id = 1 # can also be a url of Video
        self.dataset_per_subject = 50
        self.ret = False
        self.trained_model = 0

        self.image = cv2.imread("icon/face.png", 1)
        self.modified_image = self.image.copy()
        self.draw_text("Autenticar estudiante usando el reconocimiento facial", 40, 30, 1, (255,255,255))
        self.display()
        # Actions 
        ...
        # Menu
        self.about_menu = self.menu_bar.addAction("Acerca de")
        self.help_menu = self.menu_bar.addAction("Ayuda")
    ...
    def save_image(self):       # Save image captured using the save button.
        ...
        QMessageBox().about(self, "Imagen guardada", "Imagen guardada con éxito en "+location+"/"+file_name)
    ...
    def save_dataset(self):     # Save images of new dataset generated using generate dataset button.
        ...
        if self.dataset_per_subject < 1:
            QMessageBox().about(self, "Conjunto de datos generado", "Su cara ha sido capturada, puede entrenar el modelo \n o generar un nuevo conjunto de datos.")
        ...
        if self.generate_dataset_btn.isChecked():
            ...
            if len(faces) is not 1:
                self.draw_text("Solo una persona a la vez")
            else:
    ...
    def generate(self):     # Envoke user dialog and enter name and key.
        if self.generate_dataset_btn.isChecked():
            try:
                user = USER()
                user.exec_()
                name, key, email, bday = user.get_name_key()
                cursor = cnx.cursor()
                cursor.execute("use Formulario")
                sql = "INSERT INTO Alumnos (Nombre, Matricula, Correo, BDate) VALUES ('%s', '%s', '%s', '%s')" % (name, key, email, bday)
                cursor.execute(sql)
                cnx.commit()
                ...
                self.start_timer()
                self.generate_dataset_btn.setText("Generando")
            except:
                msg = QMessageBox()
                msg.about(self, "Error!", '''Complete los datos para continuar''')
                self.generate_dataset_btn.setChecked(False)
    ...
    def save_model(self):       # Save anyone model.
        try:
            ...
            msg = self.algorithm+" modelo entrenado, dejar de entrenar o entrenar a otro modelo"
            self.trained_model += 1
            self.progress_bar_train.setValue(self.trained_model)
            QMessageBox().about(self, "Entrenamiento completado", msg)
        except Exception as e:
    ...
    def train(self):        # When train button is clicked.
        if self.train_model_btn.isChecked():
            ...
            self.train_model_btn.setText("Detener entrenamiento")
            os.makedirs("training", exist_ok=True)
            labels, faces = self.get_labels_and_faces()
            try:
                msg = self.algorithm+" modelo de entrenamiento iniciado"
                QMessageBox().about(self, "Entrenamiento iniciado", msg)
    ...
    def recognize(self):        # When recognized button is called.
        if self.recognize_face_btn.isChecked():
            self.start_timer()
            self.recognize_face_btn.setText("Detener")
            self.read_model()
        else:
            self.recognize_face_btn.setText("Reconocer cara")
            self.stop_timer()
        ...
        for (x, y, w, h) in faces:
            ...
            if self.recognize_face_btn.isChecked():
                try:
                    ...
                    self.draw_text("Realizando reconociemiento en base a: "+self.algorithm, 70,50)
    ...
    def save_video(self):       # Saving video.
        if self.video_recording_btn.isChecked() and self.ret:
            ...
            if self.ret:
                QMessageBox().about(self, "Grabacion completa","Videoclip grabado con éxito en la carpeta de grabación actual")
            else:
                QMessageBox().about(self, "Informacion", '''¡Inicie la generación o el reconocimiento de conjuntos de datos primero! ''')
    ...
    def about_info(self):       # Menu Information of info button of application.
        msg_box = QMessageBox()
        msg_box.setText('''
            Aplicación de reconocimiento facial. Utiliza Machine Learning para entrenar al
             modelo generado usando el clasificador haar.
             Se implementan los algoritmos Eigenfaces, Fisherfaces y LBPH.
        ''')
    ...
    def help_info(self):       # Menu Information of help button of application.
        msg_box = QMessageBox()
        msg_box.setText('''
            Esta aplicación es capaz de crear conjuntos de datos, generar modelos,
             grabar videos y hacer clic en imágenes.
             También se implementan detección de rostro, ojos, sonrisa.
             El reconocimiento de la persona es el trabajo principal de esta aplicación.
        ''')
        msg_box.setInformativeText('''
            Siga estos pasos para usar esta aplicación 
            1. Genere al menos dos conjuntos de datos. 
            2. Entrene todo el modelo algorítmico usando los botones de radio dados. 
            3. Reconocer a la persona.
            ''')
